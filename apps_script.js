...
const ESQUEMA = {
  ...
  actividades:   ['id','nombre','deporte','categoria','profesor_id','dias_horarios','valor_mensual','cupo_maximo','estado'],
...
// SHA-256 server-side (Apps Script)
function hashSHA256(text) {
  var bytes = Utilities.computeDigest(Utilities.DigestAlgorithm.SHA_256, text, Utilities.Charset.UTF_8);
  return bytes.map(function(b) { return ('0' + (b & 0xff).toString(16)).slice(-2); }).join('');
}

// Acepta tanto contraseña en texto plano (admin la escribe en la planilla)
// como hasheada SHA-256 (la app la envía siempre hasheada).
function pwdMatch(stored, input) {
  if (!stored) return true;  // sin contraseña configurada → acceso libre
  stored = String(stored).trim();
  input  = String(input  || '').trim();
  if (!input) return false;
  if (stored === input) return true;                       // ambos son hashes
  if (!/^[0-9a-f]{64}$/.test(stored)) {                  // stored es texto plano
    return hashSHA256(stored) === input;
  }
  return false;
}

function generarPwdTemp() {
...
function doPost(e) {
  try {
    ...
    if (accion === 'login_portal' || accion === 'login_alumno') {
      ...
      if (al && pwdMatch(al.password, pwdIn)) {
      ...
      if (u && pwdMatch(u.password, pwdIn)) {
    ...
    if (accion === 'get_perfil') {
      ...
      if (!al || !pwdMatch(al.password, pwdA)) return respErr('No autorizado');
      const safe = Object.assign({}, al); delete safe.password;
    ...
    if (accion === 'update_perfil') {
      ...
      ['telefono','email','contacto_emergencia','salud','observaciones',
       'responsable_nombre','responsable_dni','responsable_vinculo','responsable_telefono'].forEach(function(k) {
    ...
    if (accion === 'reset_password') {
      const dniR = String(body.dni || '').trim();
      if (!dniR) return respErr('DNI requerido');
      const alumnos = leerHoja('alumnos');
      const al = alumnos.find(function(a) { return String(a.dni||'').trim() === dniR && a.estado !== 'baja'; });
      const usuarios = leerHoja('usuarios');
      const u = usuarios.find(function(u) { return String(u.dni||'').trim() === dniR && u.activo !== false; });
      const persona = al || u;
      const hojaN = al ? 'alumnos' : 'usuarios';
      if (!persona) return respErr('No se encontró ningún usuario con ese DNI');
      const emailDest = persona.email;
      if (!emailDest) return respErr('No tenés un email registrado. Contactá al club para recuperar tu contraseña.');
      const tempPwd = generarPwdTemp();
      actualizarFila(hojaN, persona.id, { password: tempPwd });
      const nombre = (persona.nombre || '') + (persona.apellido ? ' ' + persona.apellido : '');
      try {
        MailApp.sendEmail({
          to: emailDest,
          subject: 'Madryn Norte — Contraseña temporal',
          body: 'Hola ' + nombre + ',\n\n'
            + 'Recibimos una solicitud para recuperar tu contraseña.\n\n'
            + 'Tu contraseña temporal es: ' + tempPwd + '\n\n'
            + 'Ingresá al portal con esta contraseña y luego cambiala desde tu perfil.\n\n'
            + 'Madryn Norte'
        });
      } catch(e) { return respErr('Error al enviar el email: ' + e.message); }
      return respOk({ message: 'Se envió una contraseña temporal a tu email registrado' });
    }

    // ── Cambiar contraseña (portal) ──
    if (accion === 'cambiar_pwd') {
      const dniC     = String(body.dni      || '').trim();
      const pwdAct   = String(body.pwd      || '').trim();
      const pwdNueva = String(body.nueva_pwd|| '').trim();
      if (!dniC || !pwdAct || !pwdNueva) return respErr('Faltan datos');
      const alumnos2 = leerHoja('alumnos');
      const al2 = alumnos2.find(function(a) { return String(a.dni||'').trim() === dniC && a.estado !== 'baja'; });
      if (al2 && pwdMatch(al2.password, pwdAct)) {
        actualizarFila('alumnos', al2.id, { password: pwdNueva });
        return respOk({ message: 'Contraseña actualizada' });
      }
      const usuarios2 = leerHoja('usuarios');
      const u2 = usuarios2.find(function(u) { return String(u.dni||'').trim() === dniC && u.activo !== false; });
      if (u2 && pwdMatch(u2.password, pwdAct)) {
        actualizarFila('usuarios', u2.id, { password: pwdNueva });
        return respOk({ message: 'Contraseña actualizada' });
      }
      return respErr('La contraseña actual es incorrecta');
    }
    ...
    if (accion === 'activar_portal') {
      ...
      const usuariosAct = leerHoja('usuarios');
      const uAct = usuariosAct.find(function(u) { return String(u.id) === String(userId) && u.activo !== false; });
      if (!uAct) return respErr('Link inválido o usuario inactivo');
      const dniOcupado = usuariosAct.find(function(x) { return String(x.id) !== String(userId) && String(x.dni||'').trim() === dniNew; });
      ...
      const safeAct = Object.assign({}, uAct, { dni: dniNew }); delete safeAct.password;
      return respOk({ tipo: 'usuario', usuario: safeAct });
    }

    // ── Setup demo (temporal — crear usuarios de prueba) ──
    if (accion === 'setup_demo') {
      const PWD_ALUMNO = 'c1042ecc51482cef39f2e89e1273a35074db7f873f1ac6050efd546a9bceefc0';
      const PWD_PROFE  = '25da2ece06d8767eccaff139d4ba7490dd1b9aeeb51f413de23bce797cc47e68';
      const DNI_ALUMNO = '99999999';
      const DNI_PROFE  = '88888888';
      var alumnos = leerHoja('alumnos');
      var alDemo = alumnos.find(function(a){ return String(a.dni) === DNI_ALUMNO; });
      if (alDemo) {
        actualizarFila('alumnos', alDemo.id, { password: PWD_ALUMNO, email: 'demo.alumno@madryn.com' });
      } else {
        var newAl = { id: 'A9999', nombre: 'Demo', apellido: 'Alumno', dni: DNI_ALUMNO, estado: 'activo', password: PWD_ALUMNO, email: 'demo.alumno@madryn.com', fecha_alta: new Date().toISOString().slice(0,10) };
        insertarFila('alumnos', newAl);
        alDemo = newAl;
      }
      var usuarios2 = leerHoja('usuarios');
      var uDemo = usuarios2.find(function(u){ return String(u.dni) === DNI_PROFE; });
      if (uDemo) {
        actualizarFila('usuarios', uDemo.id, { password: PWD_PROFE });
      } else {
        var sinDni = usuarios2.find(function(u){ return u.rol === 'profesor' && (!u.dni || String(u.dni).trim() === ''); });
        if (sinDni) {
          actualizarFila('usuarios', sinDni.id, { dni: DNI_PROFE, password: PWD_PROFE });
          uDemo = sinDni;
        } else {
          var newU = { id: 'U9999', nombre: 'Demo', apellido: 'Profe', username: 'demoprofe', email: 'demo.profe@madryn.com', password: PWD_PROFE, rol: 'profesor', activo: true, dni: DNI_PROFE };
          insertarFila('usuarios', newU);
          uDemo = newU;
        }
      }
